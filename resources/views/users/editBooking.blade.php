    ...
    <div class="w-full lg:w-10/12 mx-auto bg-white border border-gray-200 rounded-2xl shadow-sm p-10">
        ...
        {{-- <div class="flex flex-col md:flex-row justify-between items-center md:items-start bg-gray-100 rounded-lg p-6 mb-8">
            <div class="flex items-center space-x-5">
                ...
                <div>
                    <h2 class="text-2xl font-semibold text-gray-800">{{ $editBooking->court->court_name }}</h2>
                    <p class="text-gray-600">{{ $editBooking->court->location ?? 'No location info' }}</p>
                </div>
            </div>
        </div> --}}
        ...
        {{-- @if($booking->court->photo)
            <div class="mb-10">
                <img src="{{ asset('storage/' . $editBooking->court->photo) }}" alt="Court Photo"
                    class="w-full rounded-lg object-cover max-h-[400px] mx-auto shadow">
            </div>
        @endif --}}

        {{-- Booking Edit Form --}}
        <form action="{{ route('editBooking', $editBooking->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="date" id="date"
                        value="{{ old('date', $editBooking->date) }}"
                ...
                <div>
                    ...
                    <input type="time" name="start_time" id="start_time"
                        value="{{ old($editBooking->time->format('H:i')) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-green-500 focus:outline-none">
                </div>
            </div>
